...
# Only import ONNX dependencies when building model
...
try:
    ...
    print("Warning: ONNX dependencies not fully available. Install onnx-ir for conversion.")
...
class Gemma3ONNXBuilder:
    """
    Builds ONNX model from a merged Gemma3 model (base + LoRA).
    Based on build_gemma.py from gemma3-exercise.
    """

    def __init__(self, merged_model, config, *, precision: str = "fp32"):
        ...
        self.node_names = set()
        self.embedding_weight_name = None
    ...
    def _build_lm_head(self, hidden_states: str):
        ...
        logits_dtype = ir.DataType.FLOAT
    ...
    def build_model(self):
        """Build the ONNX model from the merged PyTorch model."""
        self._build_inputs_and_outputs()
        seqlens_k, total_seq_len, pos_ids_reformatted = self._build_preprocessing()

        print("  Building embedding layer...")
        hidden_states = self._build_embedding(self.merged_model.model.embed_tokens.weight)

        layers = self.merged_model.model.layers
        ...
        final_norm_out = self._build_layernorm(hidden_states, self.merged_model.model.norm.weight, "/model/final_norm", is_final_norm=True)
...
def load_and_merge_model(
    ...
    """
    Load base model, apply LoRA adapter, and merge weights.
    ...
    Returns:
        Tuple of (merged_model, config, tokenizer)
    """
    print(f"Loading base model: {base_model_name}")
    base_model = AutoModelForCausalLM.from_pretrained(
        base_model_name,
        torch_dtype=torch.float32,  # Use float32 for ONNX conversion
        device_map=device,
    )
    ...
    return merged_model, config, tokenizer
...
def save_tokenizer_and_config(
    ...
    """
    Save tokenizer and config files to output directory.
    """
    ...
    # Save generation config from adapter if exists
    ...
    if os.path.exists(gen_config_path):
        ...
        # Create default generation config for translation
    ...
    # Post-process tokenizer config for chat template
    ...
    if os.path.exists(tokenizer_config_path):
        ...
        with open(tokenizer_config_path, "w", encoding="utf-8") as fp:
...
def convert_to_onnx(
    ...
    Args:
        ...
        precisions: List of precisions to export
        base_model: Base model name
    """
    if not ONNX_AVAILABLE:
        raise ImportError("ONNX dependencies not available. Please install onnx-ir, onnx, and onnxruntime.")
    ...
    merged_model, config, tokenizer = load_and_merge_model(base_model, adapter_path)

    # Save tokenizer and config
    save_tokenizer_and_config(tokenizer, config, adapter_path, output_dir)

    # Build ONNX models for each precision
    for precision in precisions:
        precision = precision.lower()
        print(f"\n{'='*60}")
        print(f"Building ONNX model with precision: {precision}")
        print(f"{'='*60}")
        ...
        builder = Gemma3ONNXBuilder(merged_model, config, precision=precision)
        builder.build_model()

        output_path = os.path.join(onnx_dir, filename)
        print(f"Saving to {output_path}...")
        builder.save_model(output_path)

    # Post-process config files for KV cache dtype
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert Gemma3 Multilingual Translator (LoRA) to ONNX format",
    ...
    parser.add_argument(
        ...
        default=["fp32", "q4"],
        help="Precision(s) to export (default: fp32 q4)"
    )

    args = parser.parse_args()

    print(f"\n{'='*60}")
    print("Gemma3 Multilingual Translator ONNX Converter")
    print(f"{'='*60}")
