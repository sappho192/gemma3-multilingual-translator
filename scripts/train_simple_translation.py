"""
Train simple translation LoRA adapter WITHOUT autotext features.

This script trains a LoRA adapter on the original Gemma-3-270m model
for multilingual translation (ko↔en, ja↔ko, en↔ja).
...
def train_simple_translation_lora(
    base_model_name: str,
    dataset_path: str,
    output_path: str,
    num_epochs: int = 3,
    ...
    """
    Train simple translation LoRA adapter.
    ...
    """
    print("\n" + "="*60)
    print("TRAINING SIMPLE TRANSLATION LORA (NO AUTOTEXT)")
    print("="*60)
...
def plot_training_results(trainer, output_path: str):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Simple Translation LoRA - Training Progress")
    plt.legend()
...
def main():
    parser = argparse.ArgumentParser(
        description="Train simple translation LoRA adapter (no autotext)"
    )
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="./models/adapters/simple-translator",
